  ...
  if (request.type === "check") {
    ...
    chrome.storage.sync.get([request.password], function (result) {
      ...
      chrome.storage.sync.set(
        { [request.password]: JSON.stringify(existingData) },
        function () {
    ...
    if (loggedIn) {
      ...
      chrome.storage.sync.get([password], function (result) {
        ...
        if (!existingEntry) {
          data[currentWebsiteTopLevel].push({
    ...
    chrome.storage.sync.get([password], function (result) {
      ...
      let data = JSON.parse(result[password]);
      data[websiteTopLevel] = data[websiteTopLevel].filter(
        (password) =>
          !(
            password.username === request.password.username &&
